...
class WordFeature(Feature):
    ...
    def create_df(self, df):
        ...
        frequent_words = mp.Manager().list([frequent_words])
...
class WordPairFeature(Feature):
    ...
    def compute_feature_matrix(self, df, X, feature_matrix):
        for _, row in tqdm(df.iterrows(), desc='Computing phrases feature_matrix... #rows=' + str(len(df)) +
            ...
            combos = list(combinations(unique_tokens, 2))
            for word in combos:
                index = self.BinarySearch(X[0], word)
                if index != -1:
                    feature_vector[index] = feature_vector[index] + 1
                #if word in X[0]:
                #    index = X[0].index(word)
    ...
    def create_df(self, df):
        ...
        frequent_phrases = mp.Manager().list([frequent_phrases])
        feature_matrix = mp.Manager().dict()
        threads = list()
        with concurrent.futures.ThreadPoolExecutor(max_workers=self.num_threads) as executor:
            for df in dfs:
...
class SyntacticGrammerFeature(Feature):

    def __init__(self, name, k, type, num_threads):
        super().__init__(name, k, type, num_threads)

    def compute_frequent_pos(self, df, trio_counter):
        ...
        for _, row in tqdm(df.iterrows(), desc='Computing frequent pos... #rows=' + str(len(df)) +
                                               ...
                                               '; cpu: ' + mp.current_process().name +
                                               '; thread: ' + str(threading.get_ident())):
            chunk_dict = chunker.chunk_sentence(row.normalized_sentence)
            trigrams_list = []
    ...
    def create_df(self, df):
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=self.num_threads) as executor:
            for df in dfs:
                t = executor.submit(self.compute_frequent_pos, df, trio_counter)
                threads.append(t)
            executor.shutdown(wait=True)
        ...
        frequent_trio_counter = sorted(list(dict(trio_counter[0].most_common(self.k)).keys()))
        frequent_trio_counter = mp.Manager().list([frequent_trio_counter])
        feature_matrix = mp.Manager().dict()
        threads = list()
        with concurrent.futures.ThreadPoolExecutor(max_workers=self.num_threads) as executor:
            for df in dfs:
                t = executor.submit(self.compute_feature_matrix, df, frequent_trio_counter, feature_matrix)
                threads.append(t)
            executor.shutdown(wait=True)
    ...
    class Chunker:
        ...
        def chunk_sentence(self, sentence: str):
            pos_tagged_sentence = SyntacticGrammerFeature.PosTagger(sentence).pos_tag()
    ...
    def compute_feature_matrix(self, df, X, feature_matrix):
        for _, row in tqdm(df.iterrows(), desc='Computing rdf2vec feature_matrix... #rows=' + str(len(df)) +
            ...
            if drug1 in X:
                ...
                feature_vector1 = list(np.random.rand(300))
            if drug2 in X:
                feature_vector2 = X[drug2]
            else:
                feature_vector2 = list(np.random.rand(300))
...
class Smiles(Feature):
    ...
    def compute_feature_matrix(self, df, X, feature_matrix):
        ...
        for _, row in tqdm(df.iterrows(), desc='Computing smiles feature_matrix... #rows=' + str(len(df)) +
            ...
            if drug1 in X[1]:
                ...
                feature_vector1 = list(np.random.rand(512))
            ...
            if drug2 in X[1]:
                ...
                feature_vector2 = list(np.random.rand(512))
            feature_vector = feature_vector1 + feature_vector2
            self.lock.acquire()
            feature_matrix[_] = feature_vector
            self.lock.release()
    ...
    def create_df(self, df):
        ...
        X = mp.Manager().list([encoder, smiles])
        dfs = np.array_split(df, self.num_threads)
        feature_matrix = mp.Manager().dict()
        threads = list()
        with concurrent.futures.ThreadPoolExecutor(max_workers=self.num_threads) as executor:
